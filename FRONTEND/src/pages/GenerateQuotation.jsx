...
// This is a mock of your existing PDF download button.
// We will reuse its logic for saved quotes.
import PuppeteerDownloadButton from '../components/PuppeteerDownloadButton';
...
const GenerateQuotation = () => {
  ...
  // --- MOCK DATA: Replace with your actual state management (e.g., Context) ---
  const [items, setItems] = useState([{ name: 'Sample Item', height: 10, length: 20, cost: 200 }]);
  const [discount, setDiscount] = useState(10);
  const grandTotal = items.reduce((acc, item) => acc + item.cost, 0) * (1 - discount / 100);
  ...
  const handleSaveQuotation = async () => {
    if (items.length === 0) {
      handleShowNotification('Cannot save an empty quotation.', 'error');
      return;
    }
    fetchMyMessages();
  }, [user]);
  ...
  const fetchMyMessages = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const payload = { items, grandTotal, discount };
      ...
      handleShowNotification('Quotation saved successfully!', 'success');
    } catch (error) {
      console.error('Failed to save quotation:', error);
      handleShowNotification(error.response?.data?.message || 'Failed to save quotation.', 'error');
    }
  };
  ...
  const handleFetchHistory = async () => {
    try {
      ...
      handleShowNotification('Failed to fetch history.', 'error');
    }
  };
  ...
  const submitComplaint = async (e) => {
    ...
    try {
      ...
      setSavedQuotations(savedQuotations.filter(q => q._id !== id));
      handleShowNotification('Quotation deleted.', 'success');
    } catch (error) {
      console.error('Failed to delete quotation:', error);
      handleShowNotification('Failed to delete quotation.', 'error');
    }
  };
  ...
  const generatePDF = async (quoteData) => {
    // This function now acts as the reusable PDF generator trigger
    try {
      const token = localStorage.getItem("token");
      // NOTE: This assumes you have a backend endpoint that can generate a PDF from data.
      // You may need to create this endpoint if your current one doesn't support it.
      // For this example, we'll pretend the existing download button's endpoint works.
      const res = await axios.post(
        "http://localhost:5000/api/puppeteer/download", // Adjust if you create a new endpoint
        quoteData,
        {
          headers: { Authorization: `Bearer ${token}` },
          responseType: "blob",
  ...
  const handleDownloadReceipt = async () => {
    ...
    for (const item of quoteItems) {
      ...
      link.href = url;
      const date = new Date(quoteData.createdAt || Date.now()).toISOString().split('T')[0];
      ...
      handleShowNotification('Download failed. Check console for details.', 'error');
    }
    ...
    <div className="container mx-auto p-4">
      ...
      <div className="bg-white p-8 rounded-2xl shadow-lg">
        <div className="flex justify-between items-center mb-6 border-b pb-4">
          <h1 className="text-2xl font-bold text-gray-800">Generate Quotation</h1>
          <div className="flex items-center gap-3">
            <button
              onClick={handleSaveQuotation}
              className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-indigo-600 rounded-lg shadow-sm transition-colors hover:bg-indigo-700 active:scale-[0.98]"
            >
              <Save size={16} />
              Save Quotation
            </button>
        ...
        {/* Your existing quotation builder UI goes here */}
        <div className="min-h-[200px]">
          <p className="text-gray-600">Your quotation items and form fields would be here.</p>
        </div>
        ...
        <div className="mt-8 flex justify-between items-center pt-4 border-t">
          <button onClick={resetList} className="text-sm text-gray-500 hover:text-red-600 transition-colors">
            Reset List
          </button>
          <button onClick={handleFetchHistory} className="text-sm text-indigo-600 hover:underline flex items-center gap-1 font-semibold">
            <History size={14} />
            View History
          </button>
        </div>
